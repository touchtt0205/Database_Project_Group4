                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $slip->slip_id }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <img src="{{ asset('storage/' . $slip->slip_path) }}" alt="Slip Image" width="100"
                                        style="cursor:pointer;">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ ucfirst($slip->status) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('admin.membership.approve', $slip->slip_id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                        <button type="submit"
                                            class="bg-green-500 text-white px-4 py-2 rounded">Approve</button>
                                    </form>
                                    <form action="{{ route('admin.membership.reject', $slip->slip_id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                        <button type="submit"
